...
import { Facebook, Figma, Github, Linkedin, Locate, Mail, PhoneCall, Send, Youtube } from 'lucide-react';
...
export default function ContactSection() {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setResult({
      success: false, message: "Sending...."
    });

    const body = {
      ...formData,
      access_key: "bd58ddc3-c56f-4a14-bccc-7e3ad1ab83d4",
    };
    ...
    if (data.success) {
      ...
      console.log(data);
      setResult({ success: false, message: "Form Submission Failed" });
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center mt-16">
            ...
            >
                ...
                {/* <div className='w-full flex items-center justify-center'><HCaptcha sitekey='bd58ddc3-c56f-4a14-bccc-7e3ad1ab83d4' onVerify={() => {}} reCaptchaCompat={false} /></div> */}
                <button
                  type="submit"
                  className="flex items-center justify-center gap-4 w-full bg-cyan-500 text-black font-semibold py-3 rounded-lg mt-2 hover:bg-cyan-400 transition"
                >
                  {/* {loading ? "Sending..." : "Send"} */}
                  <Send className='w-5 h-5' /> Send
                </button>
                {result && (
                  <div className={`text-center mt-4 text-sm ${result.success ? "bg-green-500/50" : "bg-red-500/50"} p-2 rounded-lg`}>
                    {result.message}
                  </div>
                )}
